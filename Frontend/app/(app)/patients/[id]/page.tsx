...
import { usePatientDetail, useFacilities, useDoctors, useNurses } from "@/lib/api";

const fadeUp = { initial: { opacity: 0, y: 12 }, animate: { opacity: 1, y: 0 } };
const stagger = { animate: { transition: { staggerChildren: 0.04 } } };

function lookupName(list: { id: string; name: string }[], id: string) {
...
export default function PatientProfilePage() {
  ...
  if (loading) return <div className="p-8 text-center text-[#a1a1aa]">Loading patient...</div>;
    ...
    <motion.div initial="initial" animate="animate" variants={stagger} className="p-6 space-y-6 max-w-7xl mx-auto">
      <motion.div variants={fadeUp} className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-[#f4f4f5] flex items-center justify-center text-xl font-bold text-[#18181b]">{patient.avatar || patient.name.split(" ").map((s: string) => s[0]).join("").slice(0, 2)}</div>
          <div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { label: "Facility", value: lookupName(facilities ?? [], patient.facilityId), icon: MapPin },
          { label: "Last Visit", value: patient.lastVisit, icon: Calendar },
          { label: "Next Follow-up", value: patient.nextFollowUp, icon: Clock },
          { label: "Assigned Doctor", value: lookupName(doctors ?? [], patient.assignedDoctorId), icon: Stethoscope },
          { label: "Assigned Nurse", value: lookupName(nurses ?? [], patient.assignedNurseId), icon: Users },
        ].map((s) => (
      ...
      <motion.div variants={fadeUp} className="bg-white rounded-2xl p-5 border border-[#e4e4e7]">
        ...
        <div className="space-y-4">
          {[
            { date: patient.lastVisit, doctor: lookupName(doctors ?? [], patient.assignedDoctorId), summary: "Routine checkup. Vitals stable. Prescribed medications for ongoing management.", type: "Follow-up" },
            { date: "2026-06-15", doctor: lookupName(doctors ?? [], patient.assignedDoctorId), summary: "Initial consultation. Lab work ordered. Blood pressure and sugar levels elevated.", type: "Initial Visit" },
          ].map((visit, i) => (
