...
export default function NursesPage() {
  ...
  const facilityNames = useMemo(() => Array.from(new Set(allNurses.map(n => getFacilityName(allFacilities, n.facilityId)))), [allNurses, allFacilities]);
  const villages = useMemo(() => Array.from(new Set(allNurses.flatMap(n => n.assignedVillages))), [allNurses]);

  const filtered = useMemo(() => {
    return allNurses.filter((nurse) => {
      const searchMatch = `${nurse.name} ${nurse.assignedVillages.join(" ")} ${getFacilityName(allFacilities, nurse.facilityId)}`.toLowerCase().includes(search.toLowerCase());
      const facilityMatch = filterBy === "all" || (filterBy === "facility" && (!selectedFacility || getFacilityName(allFacilities, nurse.facilityId) === selectedFacility));
  ...
  }, [search, filterBy, selectedFacility, selectedVillage, allNurses, allFacilities]);
  const nurse = selected ? allNurses.find((item) => item.id === selected) : null;
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 lg:p-8">
          ...
          {facilityNames.length > 1 && (
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                {(["all", "facility", "village"] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => { setFilterBy(f); setSelectedFacility(null); setSelectedVillage(null); }}
                    className={`px-4 py-2 rounded-xl text-label-sm font-medium transition-all duration-200 ${filterBy === f ? "bg-brand-purple text-white shadow-lg" : "bg-surface-container-lowest/70 text-outline hover:bg-outline-variant/30 hover:shadow-md hover:scale-[1.02]"}`}
                  >
                    {f === "all" ? "All" : f === "facility" ? "Facility" : "Village"}
                  </button>
                ))}
              </div>
              <div className="flex flex-wrap gap-3">
                {filterBy === "facility" && (
                  <select
                    ...
                    className="px-4 py-2 rounded-xl text-label-sm font-medium bg-surface-container-lowest/70 border border-outline-variant/20"
                  >
      ...
      <div className="card-glass overflow-hidden p-5">
                ...
                <td className="min-w-0 px-2 py-2.5">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-[11px] text-outline md:hidden">{getFacilityName(allFacilities, item.facilityId)}</p>
                    </div>
                  </div>
                </td>
            ...
            <div className="flex items-center justify-between border-b border-outline-variant/10 px-5 py-4">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-label-xs text-outline">{getFacilityName(allFacilities, nurse.facilityId)}</p>
                </div>
              </div>
