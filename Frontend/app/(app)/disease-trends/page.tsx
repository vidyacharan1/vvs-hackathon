...
import { useDiseaseSpikes, useHealthTrends, useVillageConditions, useFacilities } from "@/lib/api";
...
export default function DiseaseTrendsPage() {
  const { data: spikes } = useDiseaseSpikes();
  const { data: healthTrends } = useHealthTrends();
  const { data: villageConditions } = useVillageConditions();
  const { data: facilitiesList } = useFacilities();

  const allSpikes = spikes ?? [];
  const allHealthTrends = healthTrends ?? [];
  const allVillages = villageConditions ?? [];
  const allFacilities = facilitiesList ?? [];

  const criticalSpikes = allSpikes.filter((item) => item.risk === "critical" || item.risk === "high").length;
      ...
      <div className="card-glass p-4">
        ...
        <div className="h-[260px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={allHealthTrends} margin={{ top: 8, right: 12, left: -20, bottom: 0 }}>
              ...
              <div key={`${disease.condition}-${disease.facilityId}`} className="rounded-2xl border border-outline-variant/20 bg-white/70 p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[11px] text-outline">{getFacilityName(allFacilities, disease.facilityId)}</p>
                  </div>
            ...
            {allVillages.map((village) => (
              <div key={`${village.village}-${village.condition}`} className="flex items-center justify-between gap-3 rounded-2xl border border-outline-variant/20 bg-white/70 p-2.5">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-[11px] text-outline">{getFacilityName(allFacilities, village.facilityId)}</p>
                </div>
