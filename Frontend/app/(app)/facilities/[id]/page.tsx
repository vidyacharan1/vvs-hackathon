...
function HealthCard({ icon: Icon, label, value, status }: { icon: ElementType; label: string; value: string | number; status: FacilityStatus }) {
  return (
    <div className="card-glass min-w-0 p-3">
      ...
      <span className={`pill pill-${status} mt-2`}>{statusLabel[status]}</span>
    </div>
  );
}
...
export default function FacilityDetailPage() {
  ...
  const healthCards = [
    ...
    { icon: Stethoscope, label: "Doctors", value: `${facility.doctorsPresent}/${facility.totalDoctors} present`, status: facility.doctorsPresent < facility.totalDoctors ? "high" as FacilityStatus : "low" as FacilityStatus },
    { icon: Users, label: "Nurses", value: `${facility.nursesPresent}/${facility.totalNurses} on duty`, status: facility.nursesPresent < facility.totalNurses ? "high" as FacilityStatus : "low" as FacilityStatus },
    { icon: Pill, label: "Medicine", value: `${facility.medicineStockIssues} issues`, status: facility.medicineStockIssues >= 3 ? "critical" as FacilityStatus : facility.medicineStockIssues >= 1 ? "high" as FacilityStatus : "low" as FacilityStatus },
              ...
              {meds.length ? meds.map((medicine) => (
                <div key={medicine.id} className="rounded-2xl border border-outline-variant/20 bg-white/60 p-2.5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className={`pill pill-${medicine.risk}`}>{statusLabel[medicine.risk]}</span>
                  </div>
          ...
          <Panel title="Disease & Bed Pressure" icon={TrendingUp}>
            <div className="space-y-2">
              {[
                ...
                { label: "Open alerts", value: facility.openAlerts, icon: AlertCircle, status: facility.openAlerts >= 6 ? "critical" as FacilityStatus : facility.openAlerts >= 3 ? "high" as FacilityStatus : "low" as FacilityStatus },
              ].map((item) => (
              ...
              {insights.length ? insights.map((insight) => (
                <div key={insight.id} className="rounded-2xl border border-outline-variant/20 bg-white/60 p-2.5">
                  <div className="mb-1 flex items-center justify-between gap-2">
                    ...
                    <span className={`pill pill-${insight.severity}`}>{statusLabel[insight.severity]}</span>
                  </div>
