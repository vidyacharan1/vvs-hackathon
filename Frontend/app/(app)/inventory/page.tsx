...
import { useInventoryAlerts, useFacilities as useApiFacilities } from "@/lib/api";
...
export default function InventoryPage() {
  ...
  }, [facilityFilter, search, displayMedicines]);
    ...
    <div className="space-y-4 p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => { const med = prompt("Medicine name:"); const qty = prompt("Quantity:"); if (med && qty) { fetch("/api/v1/inventory/add-stock", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ medicine: med, facilityId: "phc-madhurawada", quantity: parseInt(qty) }) }).then(() => alert("Stock added!")); } }} className="card-glass flex items-center gap-2 rounded-xl px-3.5 py-2 text-label-sm font-semibold"><Plus className="h-4 w-4" /> Add Stock</button>
          <button onClick={() => { const med = prompt("Medicine name:"); const from = prompt("From facility ID:"); const to = prompt("To facility ID:"); const qty = prompt("Quantity:"); if (med && from && to && qty) { fetch("/api/v1/inventory/transfer", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ medicine: med, fromFacilityId: from, toFacilityId: to, quantity: parseInt(qty) }) }).then(() => alert("Stock transferred!")); } }} className="card-glass flex items-center gap-2 rounded-xl px-3.5 py-2 text-label-sm font-semibold"><ArrowRightLeft className="h-4 w-4" /> Transfer</button>
