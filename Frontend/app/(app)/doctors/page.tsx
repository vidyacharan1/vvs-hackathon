...
export default function DoctorsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return doctors.filter((doctor) => `${doctor.name} ${doctor.specialty} ${getFacilityName(doctor.facilityId)}`.toLowerCase().includes(query));
  }, [search]);
  const doc = selected ? doctors.find((doctor) => doctor.id === selected) : null;
    ...
    <div className="space-y-4 p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-label-sm text-outline">{doctors.length} doctors with attendance and load tracking.</p>
        </div>
                ...
                <td className="min-w-0 px-2 py-2.5">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-[11px] text-outline md:hidden">{getFacilityName(doctor.facilityId)}</p>
                    </div>
                  </div>
                </td>
                <td className="hidden px-2 py-2.5 text-label-sm text-outline md:table-cell"><span className="line-clamp-1">{getFacilityName(doctor.facilityId)}</span></td>
            ...
            <div className="flex items-center justify-between border-b border-outline-variant/10 px-5 py-4">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-label-xs text-outline">{doc.specialty} - {getFacilityName(doc.facilityId)}</p>
                </div>
              </div>
